...
module AdminUtilities
  ...
  def self.generate_notebooks
    ...
    UserRepository.all.each do |user|
      count += 1
      sd = Noteshare::Helper::Document::SetupDocument.new(author: user, title: "#{user.screen_name.capitalize}'s Notebook", content: @content)
      sd.make
      puts user
    end
    count
  ...
  def self.change_author_id(from_id, to_id, option='test')
    count = 0
    DocumentRepository.all.each do |doc|
      if doc.author_id == from_id.to_i
        count = count + 1
        puts "#{doc.id}: #{doc.title}"
        if option == 'fix'
          doc.author_id = to_id.to_i
          DocumentRepository.update doc
        end
      end
    end
